...
subdf = df[,c(1,2,3,4,5)] # subdf is the data with all populated fields, contiguous
...
subdf.train = subdf[1:n.train,] # train set
subdf.test = subdf[(n.train+1):n,] # test set
...
ggplot() + geom_line(aes(x=1:(n.train-1),y=diff(subdf.train$UNRATE, 1)))+
  labs(x = "Months since 1954 Jul", y = "Diff(1)") +
  theme_minimal()

# ADF Test for Stationarity
adf_test = adf.test(diff(subdf.train$UNRATE, 1))
print(adf_test) # stationary

# ACF and PACF plots
plot(acf(diff(subdf.train$UNRATE, 1)), main="ACF on Diff(1)") # some significant ACF
plot(pacf(diff(subdf.train$UNRATE, 1)), main="PACF on Diff(1)") # some significant PACF
...
# Pre-COVID-19 MSPE
calculate_mse_cutoff <- function(observed_values, predicted_values, cutoff=50) {
  observed_values = observed_values[1:cutoff]
  predicted_values = predicted_values[1:cutoff]
  squared_diff <- (observed_values - predicted_values)^2
  mse <- mean(squared_diff)
  return(mse)
}
...
summary(ols.fit) # se = 0.004094
ols.fit.hc0 = sqrt(diag(hccm(ols.fit, type="hc0")))
ols.fit.hc0 # robust beta error = 0.004602488

# * Regression 2: diff ~ 1 + FEDFUNDS + GDP + CPI + UNRATE
ols.fit = lm(diff~., data=twodf)
summary(ols.fit) # se = 4.920e-03
ols.fit.hc0 = sqrt(diag(hccm(ols.fit, type="hc0")))
ols.fit.hc0 # robust beta error = 2.745246e-03
...
summary(ols.fit) # se = 5.147e-03
ols.fit.hc0 = sqrt(diag(hccm(ols.fit, type="hc0")))
ols.fit.hc0 # robust beta error = 2.937838e-03
...
ggplot() + geom_line(aes(x=1:n.test,y=y.pred), color="blue") + 
  ...
  labs(x="Months since Oct 2012", y="Unemployment Rate") +
  theme_minimal()
...
best_lambda = cv_model$lambda.min # 0.00212585
...
cv_model = cv.glmnet(X, y, alpha = 0)
best_lambda = cv_model$lambda.min # 0.004143626
ridge_model_best = glmnet(X, y, alpha = 0, lambda = best_lambda)
...
### 4. Indicator Regime Regression
# Idea: use indicators on past drops to detect regime change
...
# choose optimal (lambda*, gap*) from the grid search
lambda = 0.1 # lambda*
gap = 5 # gap*
subsubdf.iv = subsubdf[(gap+1):nrow(subsubdf),]
subsubdf.iv$ind = diff(subsubdf$UNRATE, lag=gap)
...
error.one.star # 0.03859857 (apparent error for this regime)
...
error.two.star # 0.02491733 (apparent error for this regime)
...
mse.tot # training error = 0.0292149, which is good

indicator.pred = numeric(n.test)
classification.res = numeric(n.test)
...
# Plot of prediction vs actual and regime changes
ggplot() + geom_line(aes(x=1:n.test,y=subdf.test.cur$UNRATE[n.train:(n-1)]+indicator.pred), color="blue") +
  geom_line(aes(x=1:n.test,y=subdf.test.cur$UNRATE[(n.train+1):n]), color="green") +
  geom_line(aes(x=1:n.test,y=classification.res), color="orange") +
  labs(x="Months since 2012 Oct", y="Unemployment Rate") +
  theme_minimal()

### 5. Markov Switching Model

### 5.1 p = 1 i.e. diff(t) ~ 1 + diff(t-1)
...
plotProb(mod, which=1) # smoothed probabilities
plotProb(mod, which=2) # regime 1
plotProb(mod, which=3) # regime 2

print(mod@Coef) # coefficients in both regimes
...
for(i in 1:nrow(mod@Fit@filtProb)){
  # arbitrarily chosing 0.5 as the threshold
...
# plot of unemployment rate and regime changes
# even for the simple model, it tracks the upslope and downslopes reasonably
ggplot() + geom_line(aes(x=1:n.train,y=subdf.train$UNRATE)) +
...
for (i in n.train:nrow(subdf.v2)){
  ...
  if (mod@Fit@filtProb[nnn,1]>mod@Fit@filtProb[nnn,2]){ # check the last step state
...
# Plot of actual unemployment rate and prediction
ggplot() + geom_line(aes(x=1:n.train,y=subdf.train$UNRATE)) +
  geom_line(aes(x=1:n.train,y=c(rep(NA, 2), state.here)),color="orange")+
...
### 5.2 p = 4 i.e. diff(t) ~ 1 + diff(t-1) + diff(t-2) + diff(t-3) + diff(t-4)
subdf.train.v = subdf.train
subdf.train.v$DATE = NULL
subdf.train.v2 = subdf.train.v[1:(nrow(subdf.train.v)-1),]
subdf.train.v2$diff = diff(subdf.train.v$UNRATE, 1)
...
for (i in n.train:nrow(subdf.v2)){
  ...
  mod = msmFit(model, k=2, p=4, sw=rep(TRUE,6))
  nnn = nrow(mod@Fit@filtProb)
  if (mod@Fit@filtProb[nnn,1]>mod@Fit@filtProb[nnn,2]){ # check the last step's state
...
### 5.3 p = 4, but include all other covariates i.e. diff(t) ~ 1 + UNRATE + GDP + CPI + FEDFUNDS + diff(t-1) + diff(t-2) + diff(t-3) + diff(t-4)
subdf.v = subdf.train
subdf.v$DATE = NULL
